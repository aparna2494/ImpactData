BCCapp.controller('NetworkController', ['$rootScope', '$scope', 'flowGraph','dataservices','alertDetailsService','$state','$interval','$timeout','neo4jService','KPIDetails','$filter',
		function($rootScope,$scope, flowGraph,dataservices,alertDetailsService,$state,$interval,$timeout,neo4jService,KPIDetails,$filter) {
...
var alertCollection=$state.current.data.alertCollection
var flowCollection=$state.current.data.flowCollection


console.log(alertCollection)
     ...
     $scope.data.nodes[key].data.alert='Y';
		...
		if(key==$scope.data.nodes.length-1)
			...
			/* $scope.layout = {
  			...
  			updateStyle($rootScope.selectedtrackarraylabels)
			}
...
$scope.filterByTrack=function(trackname,selected)
{
	...
	var temp=$rootScope.selectedtrackarraylabels
	var index = temp.indexOf(trackname);
		...
		$rootScope.selectedtrackarraylabels=temp
		console.log($rootScope.selectedtrackarraylabels)
...
$scope.$watch('selectedtrackarraylabels',function(newval){
...
$scope.showdetails=function(nodeID)
{
	...
	var value=$scope.selected_component_code.toString();
	neo4jService.getNeoData(value).success( function(data)
			{
		       console.log(data);
...
$scope.selectedValue = function (selected) {
	...
	console.log($scope.data.selectedNodes)
	  if(selected!=undefined)
		  ...
		  updateAlerts($rootScope.selectedtrackarraylabels,selected.title)
		    var nodef =  $scope.searchTerm;
		    updateStyle($rootScope.selectedtrackarraylabels,nodef)
		  }
	  else
		  {
		  $scope.data.selectedNodes=$scope.data.nodes
		  updateAlerts($rootScope.selectedtrackarraylabels)
		  }
...
initdata();
			$scope.$on('$viewContentLoaded', function() {
			 ...
			 $scope.reloadData= function(){
						...
						promise= $interval(function(){
							  console.log("in reload promise")
							 $rootScope.countvar=1;
							 /*if($rootScope.countvar>1)
								 {
							 ...
							 if($state.current.name=="flow")
								...
								dataservices.alertsdataaction(alertCollection).success(
			                             function(data) {
			                            	 console.log("data");
			                            	 console.log(data);
			                            	 $scope.AlertComponentArray=[];
			...
			var resu = dataservices.flowdiagram(flowCollection).success(
                    function(data) {
                    	...
                    	angular.forEach(data[0].nodes, function(value, key) {
                    		if(value.node_fixed==true)
                        		...
                        		if($scope.trackarray.map(function(e){ return e.label}).indexOf(yy.label)<0)
                            		...
                            		//$rootScope.selectedtrackarraylabels.push(yy.label);
                        			if($rootScope.selectedtrackarraylabels.length>0 && $rootScope.selectedtrackarraylabels.indexOf(yy.label)>-1)
                        				{
                    	...
                    	console.log($scope.data.nodes);
                    	var resu = dataservices.alertsdataaction(alertCollection).success(
                                function(data) {
                                	 $scope.alerts_filtered=[];
                                	 $scope.alerts=[];
                                	angular.forEach(data, function(value, key) {
...
*/                         /*$scope.styles = [ {
		   ...
		   $scope.kpidetails= function(element) {
        	  ...
        	  console.log(element.data())
        	 KPIDetails.getKPIdetails({track_name: element.data().track_name,collection: "bp_flow_data","node_id":element.data().id}).then(function(res){
